// PDF Splitter - Aplicação JavaScript (Versão Corrigida)
class PDFSplitter {
    constructor() {
    ...
    initializeEventListeners() {
        ...
        // Permitir que a área de drop seja clicável também
        dropZone.addEventListener('click', (e) => {
            // Se o clique não foi no botão de upload, acionar o input de arquivo
    ...
    async handleFileSelect(file) {
        console.log('handleFileSelect chamado com arquivo:', file);
        ...
        try {
            ...
            this.pdfDocument = await pdfjsLib.getDocument(arrayBuffer.slice(0)).promise;
            this.currentPDF = arrayBuffer; // Manter o original para PDF-lib
    ...
    async displayPDFPreview() {
        ...
        if (!thumbnailsContainer || !pageCountElement) {
    ...
    updateSelectionInfo() {
        ...
        if (!selectionInfo || !selectedPagesText) return;
    ...
    handleSplitMethodChange() {
        ...
        if (specificOptions) specificOptions.classList.add('hidden');
        ...
        switch (splitMethod) {
            case 'range':
                ...
                break;
            case 'specific':
                if (specificOptions) specificOptions.classList.remove('hidden');
                break;
            case 'size':
    ...
    calculateRangeSplit() {
        ...
        const pagesPerSplit = parseInt(pagesPerSplitInput ? pagesPerSplitInput.value : '1') || 1;
    ...
    async calculateSizeSplit() {
        const maxSizeMBInput = document.getElementById('max-size-mb');
        const maxSizeMB = parseFloat(maxSizeMBInput ? maxSizeMBInput.value : '5') || 5;
    ...
    displayResults() {
        ...
        document.querySelectorAll('.download-fragment-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const fragmentIndex = parseInt(e.target.closest('.download-fragment-btn').dataset.fragmentIndex);
    ...
    resetApplication() {
        ...
        if (fragmentsContainer) fragmentsContainer.innerHTML = '';
        ...
        const rangeRadio = document.querySelector('input[name="split-method"][value="range"]');
        if (rangeRadio) {
    ...
    validateInputs() {
        const splitMethodRadio = document.querySelector('input[name="split-method"]:checked');
        if (!splitMethodRadio) {
            this.showError('Por favor, selecione um método de fragmentação.');
            return false;
        }
        ...
        switch (splitMethod) {
            case 'range':
                const pagesPerSplitInput = document.getElementById('pages-per-split');
                const pagesPerSplit = parseInt(pagesPerSplitInput ? pagesPerSplitInput.value : '0');
            ...
            case 'size':
                const maxSizeInput = document.getElementById('max-size-mb');
                const maxSize = parseFloat(maxSizeInput ? maxSizeInput.value : '0');
    ...
    handleSpecificErrors(error) {
        if (error.message.includes('Invalid PDF')) {
            ...
            return 'Erro ao carregar biblioteca PDF.js. Verifique sua conexão com a internet.';
        } else if (error.message.includes('PDF-lib não está carregado')) {
            return 'Erro ao carregar biblioteca PDF-lib. Verifique sua conexão com a internet.';
        } else if (error.message.includes('JSZip não está carregado')) {
            return 'Erro ao carregar biblioteca JSZip. Verifique sua conexão com a internet.';
        }
        
        return 'Erro inesperado ao processar o PDF: ' + error.message;
    }
    ...
    initializeKeyboardSupport() {
        document.addEventListener('keydown', (e) => {
            ...
            if (e.ctrlKey && e.key === 'a' && this.pdfDocument) {
                e.preventDefault();
                const specificRadio = document.querySelector('input[name="split-method"][value="specific"]');
                if (specificRadio) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('Bibliotecas carregadas:', librariesLoaded);
    
    // Inicializar aplicação
    const app = new PDFSplitter();
    app.initializeKeyboardSupport();
    
    // Tornar a instância disponível globalmente para depuração
    window.pdfSplitterApp = app;
    
    console.log('PDF Splitter inicializado com sucesso');
});
